...
from torchvision.models import resnet50, densenet201
...
class CustomDataset_TTA(Dataset):
    def __init__(self, transform, mode='train', n_tta=None):
        self.transform = transform
        self.n_tta = n_tta
    ...
    def __getitem__(self, idx):
        if self.transform:
            if self.n_tta:
                imgs = [self.transform(self.img_list[idx]) for _ in range(self.n_tta)]
                return imgs
            else:
                return self.transform(self.img_list[idx]), self.img_labels[idx]
...
test_set = CustomDataset_TTA(transform=transform_train, mode='test', n_tta=5)
...
class Model(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.resnet = densenet201(pretrained=False)
...
model = torch.load("models/dense_batch64_imgsize112_epoch196_acc88.73.pt")
...
for i, images in tqdm(enumerate(test_loader)):
    output = np.array([model(image.to(device)).cpu().detach().numpy() for image in images])
